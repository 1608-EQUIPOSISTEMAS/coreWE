import nodemailer from 'nodemailer'

const transport = nodemailer.createTransport({
  host: 'smtp.zeptomail.com',
  port: 587,
  auth: {
    user: 'emailapikey',
    pass: process.env.ZEPTOMAIL_API_KEY || ''
  }
})

const transportFico = nodemailer.createTransport({
  host: 'smtp.zeptomail.com',
  port: 587,
  auth: {
    user: 'emailapikey',
    pass: process.env.ZEPTOMAIL_FICO_API_KEY || ''
  }
})

const FROM_EMAIL = process.env.ZEPTOMAIL_FROM_EMAIL || 'noreply@we-educacion.com'
const FROM_NAME = process.env.ZEPTOMAIL_FROM_NAME || 'WE Educacion Ejecutiva'

export async function sendEmail ({ to, subject, htmlBody, replyTo, attachments, fromEmail, fromName, cc, bcc }) {
  if (!process.env.ZEPTOMAIL_API_KEY) {
    console.warn('[ZeptoMail] API key no configurada, email no enviado')
    return { success: false, error: 'API key no configurada' }
  }

  // Permitir override del sender por llamada (ej. pagos@we-educacion.com para membresias).
  const finalFromEmail = fromEmail || FROM_EMAIL
  const finalFromName  = fromName  || FROM_NAME

  try {
    const info = await transport.sendMail({
      from: `"${finalFromName}" <${finalFromEmail}>`,
      to,
      subject,
      html: htmlBody,
      ...(replyTo && { replyTo }),
      ...(cc && { cc }),
      ...(bcc && { bcc }),
      ...(Array.isArray(attachments) && attachments.length > 0 && { attachments })
    })
    const ccTag = cc ? ` (cc: ${Array.isArray(cc) ? cc.join(',') : cc})` : ''
    console.log(`[ZeptoMail] Email enviado a ${to}${ccTag} desde ${finalFromEmail} | messageId: ${info.messageId}`)
    return { success: true, messageId: info.messageId }
  } catch (err) {
    console.error(`[ZeptoMail] Error enviando a ${to}:`, err.message)
    return { success: false, error: err.message }
  }
}

export async function sendFicoEmail ({ to, subject, htmlBody, cc }) {
  if (!process.env.ZEPTOMAIL_FICO_API_KEY) {
    console.warn('[ZeptoMail-FICO] API key no configurada, email no enviado')
    return { success: false, error: 'FICO API key no configurada' }
  }

  try {
    const info = await transportFico.sendMail({
      from: `"${FROM_NAME}" <${FROM_EMAIL}>`,
      to,
      subject,
      html: htmlBody,
      ...(cc && { cc })
    })
    const ccTag = cc ? ` (cc: ${Array.isArray(cc) ? cc.join(',') : cc})` : ''
    console.log(`[ZeptoMail-FICO] Email enviado a ${to}${ccTag} | messageId: ${info.messageId}`)
    return { success: true, messageId: info.messageId }
  } catch (err) {
    console.error(`[ZeptoMail-FICO] Error enviando a ${to}:`, err.message)
    return { success: false, error: err.message }
  }
}
