...
describe('excludeHeld', () => {
  it('nombra a cada inscripcion retenida y tambien a sus hijas', () => {
    const sql = excludeHeld([111, 222])
    for (const id of [111, 222]) expect(sql).toContain(String(id))
    expect(sql).toContain('e.enrollment_id NOT IN')
    expect(sql).toContain('COALESCE(e.parent_enrollment_id, 0) NOT IN')
  })

  it('nunca produce un NOT IN () vacio', () => {
    expect(excludeHeld([])).toBe('')
...
describe('OS/OP sin cobrar', () => {
  ...
  // Con la lista vacia no hay predicado que buscar; vuelve a vigilar en cuanto
  // alguien retenga un id.
  it.skipIf(HELD_ENROLLMENT_IDS.length === 0)('toda hoja sigue respetando la retencion manual', () => {
