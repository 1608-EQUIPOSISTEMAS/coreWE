...
export function resolveDestKind ({ originProgramVersionId, destProgramVersionId, destEditionId }) {
  if (!destProgramVersionId) throw new ReprogramacionError('Falta el programa destino')
  const kind = Number(originProgramVersionId) === Number(destProgramVersionId) ? 'RP' : 'CC'
...
export function assertPuedeAceptar (caso) {
  ...
  if (!caso?.dest_edition_id && !caso?.dest_program_version_id) {
